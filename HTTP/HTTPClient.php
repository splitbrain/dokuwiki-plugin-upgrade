...
class HTTPClient
{
        ...
        try {
            ...
            if (isset($this->resp_headers['set-cookie'])) {
                foreach ((array) $this->resp_headers['set-cookie'] as $cookie) {
                    [$cookie] = array_pad(explode(';', $cookie, 2), 2, '');
                    [$key, $val] = array_pad(explode('=', $cookie, 2), 2, '');
                    $key = trim($key);
                    if ($val == 'deleted') {
    ...
    protected function sendData($socket, $data, $message)
    {
        ...
        while ($written < $towrite) {
            // check timeout
            $time_used = static::time() - $this->start;
    ...
    {
        echo '<b>' . $info . '</b> ' . (static::time() - $this->start) . 's<br />';
        if (!is_null($var)) {
            ob_start();
            print_r($var);
    ...
    {
        echo '*' . $info . '* ' . (static::time() - $this->start) . "s\n";
